	...
	<div align="center">
		...
		<c:if test="${count > 0}">
			<c:if test="${startPage > clickablePageCount }">
				<a href="/board/boardList?pageNum=${startPage - clickablePageCount}">[이전]</a>
			</c:if>

			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="/board/boardList?pageNum=${i}">[${i}]</a>
			</c:forEach>

			<c:if test="${endPage < pageCount }">
				<a href="/board/boardList?pageNum=${startPage + clickablePageCount}">[다음]</a>
			</c:if>
		</c:if>
	</div>
</body>
